...
const PI_SLOTS = [
  {key:'w7',conf:'west',label:'OESTE — 7º SEED'},{key:'w8',conf:'west',label:'OESTE — 8º SEED'},
  {key:'w9',conf:'west',label:'OESTE — 9º SEED'},{key:'w10',conf:'west',label:'OESTE — 10º SEED'},
  {key:'e7',conf:'east',label:'LESTE — 7º SEED'},{key:'e8',conf:'east',label:'LESTE — 8º SEED'},
  {key:'e9',conf:'east',label:'LESTE — 9º SEED'},{key:'e10',conf:'east',label:'LESTE — 10º SEED'},
];
...
function renderPlayinTeamEditor(){
  ...
  document.getElementById('pi-teams-editor').innerHTML=`
    <div class="pi-game"><div class="pi-game-head">CONFERÊNCIA OESTE</div>${slotRows(west)}</div>
    <div class="pi-game"><div class="pi-game-head">CONFERÊNCIA LESTE</div>${slotRows(east)}</div>`;
}
...
const PO_ROUND_DEFS = [
  ...
  {id:'r2',label:'SEMIFINAIS',matches:[
    {mk:'wR2_0',conf:'west'},{mk:'wR2_1',conf:'west'},{mk:'eR2_0',conf:'east'},{mk:'eR2_1',conf:'east'},
  ]},
  {id:'r3',label:'FINAIS DE CONFERÊNCIA',matches:[
    {mk:'wR3_0',conf:'west'},{mk:'eR3_0',conf:'east'},
  ]},
  {id:'finals',label:'NBA FINALS',matches:[{mk:'finals',conf:'champ'}]},
];
...
function renderSistema(){
  ...
  el.innerHTML=`
    ...
    Resultados Play-In: <b style="color:var(--text)">${piResults}/6</b><br>
    Resultados Playoffs: <b style="color:var(--text)">${poResults}/15</b><br>
  ...
  reader.onload=async e=>{
    try{
      ...
      if(data.picks){
        ...
        Object.entries(data.picks).forEach(([pid,picks])=>{
          batch.set(doc(db,'picks',pid),picks);
        });
        await batch.commit();
      }
